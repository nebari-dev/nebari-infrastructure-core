...
- For a custom local path (`repository.local.path`), you must also declare a matching `cluster.local.kind.extra_mounts` entry so the kind nodes can read it. See `examples/local-config.yaml`.
