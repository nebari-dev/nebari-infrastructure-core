...
```bash
pixi workspace channel add https://prefix.dev/github-releases
...
```bash
pixi global install -c https://prefix.dev/github-releases nebari-infrastructure-core
```

Both pixi paths resolve `nic` from prefix.dev's shared [`github-releases`](https://prefix.dev/channels/github-releases/packages/nebari-infrastructure-core) channel, which is generated by repackaging published GitHub releases on a two-hourly cron. A release cut in the last couple of hours may not be resolvable yet; the release archives and Homebrew below are available immediately.
