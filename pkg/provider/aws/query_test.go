...
func TestQuery_RequiresRegion(t *testing.T) {
	p := NewProvider()
	ctx := context.Background()

	_, err := p.Query(ctx, "test-cluster")
	if err == nil {
		t.Fatal("Query() should return error when region is not provided")
	}

	expectedMsg := "Query requires region parameter"
	if err.Error()[:len(expectedMsg)] != expectedMsg {
		t.Errorf("Expected error message to start with %q, got %q", expectedMsg, err.Error())
	}
}

func TestConvertToProviderState_Minimal(t *testing.T) {
	...
	state := convertToProviderState(clusterName, region, nil, cluster, nil, nil)

	if state == nil {
		t.Fatal("convertToProviderState returned nil")
	}

	if state.ClusterName != clusterName {
		t.Errorf("Expected cluster name %s, got %s", clusterName, state.ClusterName)
	}
	...
	if state.Region != region {
		t.Errorf("Expected region %s, got %s", region, state.Region)
	}
	...
	if state.Cluster.Name != clusterName {
		t.Errorf("Expected cluster name %s, got %s", clusterName, state.Cluster.Name)
	}
...
func TestConvertToProviderState_WithVPC(t *testing.T) {
	clusterName := "test-cluster"
	region := "us-west-2"
	...
	state := convertToProviderState(clusterName, region, vpc, cluster, nil, nil)
	...
	// Should have 4 total subnets (2 public + 2 private)
...
func TestConvertToProviderState_WithNodeGroups(t *testing.T) {
	clusterName := "test-cluster"
	region := "us-west-2"
	...
	state := convertToProviderState(clusterName, region, nil, cluster, nodeGroups, nil)
...
func TestConvertToProviderState_NilInputs(t *testing.T) {
	state := convertToProviderState("test", "us-west-2", nil, nil, nil, nil)
...
func TestConvertToProviderState_ClusterMetadata(t *testing.T) {
	cluster := &ClusterState{
		Name:                "test",
	...
	state := convertToProviderState("test", "us-west-2", nil, cluster, nil, nil)
...
func TestConvertToProviderState_EmptyNodeGroups(t *testing.T) {
	cluster := &ClusterState{
		Name:    "test",
		Version: "1.28",
		Status:  "ACTIVE",
	...
	emptyNodeGroups := []NodeGroupState{}

	state := convertToProviderState("test", "us-west-2", nil, cluster, emptyNodeGroups, nil)
...
func TestProviderInfrastructureStateType(t *testing.T) {
	// Test that our conversion produces the correct provider.InfrastructureState type
	cluster := &ClusterState{
		Name:    "test",
		Version: "1.28",
	}
	...
	// This should compile - verifies we're returning the correct type
	var _ = state
