...
// ROSA runs on AWS, so provision-mode OpenTofu state lives in an S3 bucket,
// mirroring the aws provider's remote-state pattern (pkg/providers/cluster/aws/
// state.go). This is required, not optional: tofu.Setup extracts the templates
// into a temp dir that is removed after each command, so local state would not
// survive between `deploy` and a later `destroy`.

// See https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html
const maxBucketNameLength = 63

// attrKeyRegion is the tracing attribute key for the AWS region.
const attrKeyRegion = "region"
...
// generateBucketName creates a deterministic bucket name from account ID, region,
// and project name. The account ID is hashed to avoid exposing it directly. The
// "nic-ocp-tfstate" prefix keeps OpenShift state distinct from the aws provider's
// "nic-tfstate" buckets so the same project name can target both without a clash.
func generateBucketName(accountID, region, projectName string) (string, error) {
	...
	name := fmt.Sprintf("nic-ocp-tfstate-%s-%s-%s", projectName, region, suffix)
...
func ensureStateBucket(ctx context.Context, client S3Client, region, bucketName string) error {
	...
	_, span := tracer.Start(ctx, "openshift.EnsureStateBucket")
	defer span.End()
...
func destroyStateBucket(ctx context.Context, client S3Client, region, bucketName string) error {
	tracer := otel.Tracer("nebari-infrastructure-core")
	_, span := tracer.Start(ctx, "openshift.DestroyStateBucket")
	defer span.End()
