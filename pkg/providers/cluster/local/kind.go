...
func kindClusterExists(kp *cluster.Provider, name string) (bool, error) {
...
func createKindCluster(kp *cluster.Provider, name string, kindCfg *KindConfig) error {
...
func kindNodeAddressPool(kp *cluster.Provider, name string) (string, error) {
	nodeList, err := kp.ListNodes(name)
	if err != nil {
		return "", fmt.Errorf("list nodes for kind cluster %s: %w", name, err)
	}
	if len(nodeList) == 0 {
		return "", fmt.Errorf("kind cluster %s has no nodes", name)
	}
	...
	if ipv4 == "" {
		return "", fmt.Errorf("kind cluster %s node has no IPv4 address", name)
	}
