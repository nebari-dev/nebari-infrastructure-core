...
// detail (status.MetadataKeyDetail) so consumers can decode any sub-field
...
// typed fields, and forwards the full event as the detail. Lines that
...
func jsonLineMapper(line []byte) status.Update {
	...
	// Copy the line so the detail isn't aliasing the Writer's internal
	// buffer (which gets reused on the next Write).
	detail := make(json.RawMessage, len(line))
	copy(detail, line)
	return status.NewUpdate(mapStatusLevel(ev.Level), ev.Message).
		WithMetadata(status.MetadataKeyDetail, detail)
}
