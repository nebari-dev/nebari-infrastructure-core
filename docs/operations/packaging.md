...
**Publication is asynchronous.** octoconda runs on a two-hourly cron and its runs
finish in well under ten minutes, so a freshly cut release becomes resolvable
roughly an hour after the tag on average and about two hours at worst. Nothing in
this repository can make that faster, and no job here waits for it.

Only stable releases appear. octoconda skips prereleases, and conda forbids `-` in
a version string anyway, so a release candidate could not be packaged under its own
name even if the policy changed.
...
bridge while the upstream onboarding PR was open. That PR merged, the shared
channel now carries the full back catalogue from 0.4.0 onward, and the bridge -
`packaging/conda/` and the `publish-prefix-dev` job - has been deleted rather than
kept as a second source of packages.
...
`.github/workflows/publish-starters.yml`, which `release.yml` fires once and a
three-hourly cron re-fires until the channel catches up.

Every run resolves the tag, skips when the channel does not have that version yet,
skips when quay already carries the tag, and only then builds and publishes. So a
missed release heals on the next tick, and a blind cron cannot republish or
double-publish.
...
a skip is reported as a notice naming the version and the reason.
