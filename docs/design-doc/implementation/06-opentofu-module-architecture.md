...
4. Implement `InfraSettings(cfg)` to return provider-shaped capabilities (`StorageClass`, `GatewayHostAddress`, `LoadBalancerAnnotations`, `KeycloakBasePath`, `HTTPSPort`, etc.). Do not add `switch` statements on provider name elsewhere in the codebase.
