...
|---|---|---|
| CRD + controller (KubeRay) | The tenant's RayJobs, RayClusters and pods. The operator is deployed once and shared | The controller is multi-tenant by construction: it reconciles objects wherever they appear, and Kubernetes already provides per-namespace RBAC, quota and admission |
...
- Autoscaling is refused at admission. `enableInTreeAutoscaling: true` makes the operator create a Role and RoleBinding for the head's ServiceAccount, which admission has pinned to `runner`, granting it `patch` on pods and rayclusters in the namespace, and inject a sidecar from `autoscalerOptions.image` that no rule ever sees. A pod template can also set `automountServiceAccountToken: true`, which overrides the ServiceAccount's `false`. Both fields are refused, so `runner` stays an identity with no API rights and no token; autoscaling gets a designed identity before it is allowed.
...
One thing is open. The launcher UI proxies to a fixed Service, so a second tenant's reads through the public host are refused by RBAC and IAM (it fails closed) until callers are routed to their own copy.

## Deciding for the next pack

Ask these in order and stop at the first yes.

1. **Does it ship a controller that reconciles CRDs?** Share it. Add its kinds to `tenant-member` and the admission policy, add any object counts to the quota, add the ports its clients need to the NetworkPolicy, and refuse at admission any field that makes the controller bind RBAC to the workload identity or inject containers the policy does not see (KubeRay's autoscaling is the model). Do not deploy a second copy of the controller.
...
3. **Does it have a user model but read or write cloud resources as itself, scoped per tenant?** A copy per tenant, for the credential. Keep authorization in Kubernetes through impersonation or a verified token, bind each copy's Kubernetes rights to its own tenant only, and budget for routing callers to their copy.
...
- **Copies scale with tenant count, not usage.** Each tenant costs a namespace, a Deployment, a MongoDB, a PVC and a Keycloak client, plus an API Deployment for the launcher. Fine for a handful of teams; wrong if a tenant is ever a project.
...
6. A RayJob in `K8sJobMode` without `submitterPodTemplate` is refused; one with `enableInTreeAutoscaling: true` is refused; one whose head template sets `automountServiceAccountToken: true` is refused; an operator-created RayCluster with a disallowed image is refused.
