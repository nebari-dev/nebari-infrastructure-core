...
2. ✅ Deploy foundational software via ArgoCD: cert-manager, cluster-issuers, certificates, trust-manager, trust-bundle, Envoy Gateway, gateway-config, httproutes, securitypolicies, postgresql, CloudNativePG, Keycloak, longhorn-backup (where configured), OpenTelemetry Collector, nebari-operator, nebari-landingpage - each scoped to a `foundational` ArgoCD AppProject
