...
| StorageClass | `storage_class` | string | No | StorageClass is the default Kubernetes StorageClass for persistent volumes. When Longhorn is enabled below and this is left unset, "longhorn" is used instead of the default. Defaults to `standard`. |
