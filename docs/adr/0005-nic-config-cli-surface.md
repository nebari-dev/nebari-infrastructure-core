...
## Update: JSON Schema shipped as a docgen emitter

The update above recorded that "the `cmd/schemagen` / `schemas/` pipeline
described in the Context did not ship" and that NIC emits finished markdown
"rather than NIC emitting JSON Schema for `nebari-docs` to render into markdown
downstream". Both halves of that sentence need correcting, and one does not:

- **`cmd/schemagen` still does not exist, and will not.** The prescription from
  the previous update - "a docgen output format, not a second binary" - is what
  shipped. `schemas/` is generated by a third emitter inside `cmd/docgen`,
  behind the same `make docs` and the same CI drift gate as the two markdown
  outputs.
- **`schemas/` does ship, and `nebari-docs` does render it.** JSON Schema for
  `nebari-config.yaml` and for each registered provider is generated from the Go
  types and committed under `schemas/`. This is in addition to, not instead of,
  the markdown reference: the markdown pages remain what the docs site consumes
  for prose, and the schemas are what it renders for the structured reference.
- **Provider discovery for schemas is registry-driven, not glob-driven.** The
  markdown emitter finds provider config *files* by glob; the schema emitter
  enumerates registered *providers* through the registry, reading each one's
  config type from an optional `ConfigTyped` capability. Out-of-tree providers
