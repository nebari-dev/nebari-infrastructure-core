...
Proposed (2026-03-12) · Amended (2026-08-03): parts of the Design section did not ship as written; see [Update](#update-2026-08-shipped-posture-and-the-spin-design).
...
## Update (2026-08): shipped posture and the spin design

Parts of the Design section did not ship as written and are superseded by what landed:

- Software packs deploy into the **`nebari-apps`** ArgoCD project created by NIC's bootstrap ([#458](https://github.com/nebari-dev/nebari-infrastructure-core/issues/458)), not a dedicated `software-packs` project. `nebari-apps` uses `sourceRepos: '*'` at the standard posture; the operator-declared source allow-list is future hardening ([#530](https://github.com/nebari-dev/nebari-infrastructure-core/issues/530), [ADR-0010](0010-high-security-mode.md)).
- There is no `software_packs:` key in `NebariConfig` and no `pkg/softwarepack` package today. The pack install surface is being designed in [Software Pack Spins](../design-doc/implementation/18-software-pack-spins.md) instead: generated pack Applications are operator-owned files under a top-level `user-apps/` directory, synced by an NIC-owned `apps/user-apps-root.yaml` app-of-apps — not entries in `config.yaml` regenerated by NIC.
- The sync-wave numbers in this Design (root at 6, packs at 10) are superseded by the spin design's two-sequence model: `user-apps-root` takes a wave after the foundational waves, and pack Applications use their own sequence ordered only against each other.

What remains authoritative: the layered value-wiring model (conventions + `nebari-integration.yaml` + user overrides) is still the documented contract for how an individual pack publishes its integration surface, and [ADR-0008](0008-otel-collector-software-pack-override-point.md) continues to bind the pack-integration model to this ADR. The spin design's `wiring`/`outputs` schema is spin-level composition metadata layered on top of that per-pack interface, not a replacement for it.
