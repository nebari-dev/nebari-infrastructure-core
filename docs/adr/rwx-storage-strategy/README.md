...
**This settles *whether*, not *how much*.** `/shared` is the one place RWX is genuinely forced, but it is not the only candidate consumer and probably not the largest. Home volumes are the bigger question: nothing requires RWX there, but putting homes on it is the single move that would remove the per-user node pin and the cross-AZ constraint together, and take Longhorn off AWS with them — which is why it gets [its own topic](#topic-home-volume-access-mode) rather than a footnote here. Cross-namespace sharing between packs is a third candidate. Read this section as establishing the floor.
