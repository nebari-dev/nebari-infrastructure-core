...
- The local gateway never sees the real client address. Docker's port publishing presents loopback clients as the kind network's bridge gateway, and when Envoy runs on a worker, `externalTrafficPolicy: Cluster` has kube-proxy rewrite the source again to the publishing node's address. Cloud `LoadBalancer` services keep Envoy Gateway's default `Local` and do see the client address, so anything keyed on client IP (SecurityPolicy IP allowlists, IP-based rate limits) behaves differently on local than on cloud.
