...
Values files live under `values/`, a new top-level directory in the gitops repo, rather than under `apps/`. The root app-of-apps Application syncs `apps/` recursively as raw Kubernetes manifests; putting YAML values files there would have ArgoCD try to apply them as resources.
