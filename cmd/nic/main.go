...
func init() {
	...
	if err := globalRegistry.RegisterClusterProvider(ctx, "aws", aws.NewProvider()); err != nil {
		log.Fatalf("Failed to register AWS provider: %v", err)
	}

	if err := globalRegistry.RegisterClusterProvider(ctx, "gcp", gcp.NewProvider()); err != nil {
		log.Fatalf("Failed to register GCP provider: %v", err)
	}

	if err := globalRegistry.RegisterClusterProvider(ctx, "azure", azure.NewProvider()); err != nil {
		log.Fatalf("Failed to register Azure provider: %v", err)
	}

	if err := globalRegistry.RegisterClusterProvider(ctx, "local", local.NewProvider()); err != nil {
		log.Fatalf("Failed to register local provider: %v", err)
	}

	if err := globalRegistry.RegisterClusterProvider(ctx, "hetzner", hetzner.NewProvider()); err != nil {
		log.Fatalf("Failed to register Hetzner provider: %v", err)
	}

	// Register DNS providers explicitly
	if err := globalRegistry.RegisterDNSProvider(ctx, "cloudflare", cloudflare.NewProvider()); err != nil {
